...
#dict = {}
#pour chq ressource ci 
    #dict[ci] = np.zeros(T_max)
#pour chaq intervention 
    #st_idx = st_i - 1 
    #delta_i_t = int(intervention["delta"][st_idx]
    #pour chque resssource cj  
        #pour t entre ( st_idx , st_idx + delta_i_t)
            #dict[ci][t] +=  intervention["workload"][cj][ str(t+1)][str(st_i)] 
# pour chaque ressource, dict[cj][t] = la somme à l'instant t, sur l'ensemble des interventions, de la ressource dont on aura besoin en sachant que l'intervention commence à st_i 
